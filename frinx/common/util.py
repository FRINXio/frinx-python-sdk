...
def remove_empty_elements_from_root_dict(any_dict: DictAny) -> DictAny:
    ...
    """
    return dict((k, v) for k, v in any_dict.items() if v is not None and v != '') # noqa PLC1901
    ...
    def recursive_cleanup(d: DictAny) -> Any:
        ...
        for k, v in d.items():
            match v:
                ...
                case list():
                    cleaned[k] = [item for item in v if item is not None and item != ''] # noqa PLC1901
                case _:
                    if v is not None and v != '': # noqa PLC1901
                        cleaned[k] = v
        return cleaned
